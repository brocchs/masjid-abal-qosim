<?php

namespace App\Http\Controllers;

use App\Models\Zakat;
use App\Models\Muzakki;
use Illuminate\Http\Request;
use Illuminate\Support\Facades\Auth;

class ZakatController extends Controller
{
    public function index()
    {
        $zakats = Zakat::orderBy('created_at', 'desc')->paginate(10);
        $totalZakatFitrah = Zakat::where('jenis_zakat', 'fitrah')->where('jenis_bayar', 'tunai')->sum('total_bayar');
        $totalZakatMaal = Zakat::where('jenis_zakat', 'maal')->sum('total_bayar');
        $totalZakatShodaqoh = Zakat::where('jenis_zakat', 'shodaqoh')->sum('total_bayar');
        $totalJiwaFitrah = Zakat::where('jenis_zakat', 'fitrah')->sum('jumlah_jiwa');
        $totalJiwaMaal = Zakat::where('jenis_zakat', 'maal')->sum('jumlah_jiwa');
        $totalJiwaShodaqoh = Zakat::where('jenis_zakat', 'shodaqoh')->sum('jumlah_jiwa');
        $totalBeras = Zakat::where('jenis_bayar', 'beras')->sum('jumlah_beras');
        $totalSemua = $totalZakatFitrah + $totalZakatMaal + $totalZakatShodaqoh;
        
        return view('zakat.index', compact('zakats', 'totalZakatFitrah', 'totalZakatMaal', 'totalZakatShodaqoh', 'totalJiwaFitrah', 'totalJiwaMaal', 'totalJiwaShodaqoh', 'totalBeras', 'totalSemua'));
    }

    public function create()
    {
        return view('zakat.create');
    }

    public function store(Request $request)
    {
        $rules = [
            'nama_pembayar' => 'required|string|max:255',
            'jenis_zakat' => 'required|in:fitrah,maal,shodaqoh',
            'no_whatsapp' => 'nullable|string|max:20',
            'tanggal_bayar' => 'required|date',
        ];

        if ($request->jenis_zakat == 'fitrah') {
            $rules['jumlah_jiwa'] = 'required|integer|min:1';
            $rules['jenis_bayar'] = 'required|in:tunai,beras';
            $rules['jumlah_beras'] = 'required_if:jenis_bayar,beras|nullable|numeric|min:0';
            $rules['harga_per_jiwa'] = 'required_if:jenis_bayar,tunai|nullable|numeric|min:0';
            $rules['muzakkis'] = 'required|array|min:1';
            $rules['muzakkis.*.nama'] = 'required|string|max:255';
            $rules['muzakkis.*.hubungan_keluarga'] = 'required|string|max:255';
        } else {
            $rules['jumlah_jiwa'] = 'nullable|integer|min:0';
            $rules['total_bayar'] = 'required|numeric|min:0';
            $rules['keterangan'] = 'nullable|string';
            $rules['muzakkis'] = 'required|array|min:1';
            $rules['muzakkis.*.nama'] = 'required|string|max:255';
            $rules['muzakkis.*.hubungan_keluarga'] = 'required|string|max:255';
        }

        $request->validate($rules);

        $data = $request->only(['nama_pembayar', 'jenis_zakat', 'no_whatsapp', 'keterangan', 'jumlah_jiwa']);
        $data['tanggal_bayar'] = $request->tanggal_bayar . ' ' . date('H:i:s');
        $data['user_id'] = Auth::id();

        if ($request->jenis_zakat == 'fitrah') {
            $data['jumlah_jiwa'] = $request->jumlah_jiwa;
            $data['jenis_bayar'] = $request->jenis_bayar;
            $data['jumlah_beras'] = $request->jumlah_beras;
            $data['harga_per_jiwa'] = $request->harga_per_jiwa;
            
            if ($request->jenis_bayar == 'tunai') {
                $data['total_bayar'] = $request->jumlah_jiwa * $request->harga_per_jiwa;
            } else {
                $data['total_bayar'] = 0;
                $data['harga_per_jiwa'] = 0;
            }
        } else {
            $data['total_bayar'] = $request->total_bayar;
            $data['jumlah_jiwa'] = $request->jumlah_jiwa ?? 0;
            $data['jenis_bayar'] = 'tunai';
            $data['harga_per_jiwa'] = 0;
            $data['jumlah_beras'] = 0;
        }

        $zakat = Zakat::create($data);

        if ($request->muzakkis) {
            foreach ($request->muzakkis as $muzakki) {
                Muzakki::create([
                    'zakat_id' => $zakat->id,
                    'nama' => $muzakki['nama'],
                    'hubungan_keluarga' => $muzakki['hubungan_keluarga']
                ]);
            }
        }

        // Kirim pesan WhatsApp
        $phoneNumber = $zakat->no_whatsapp ? '62' . ltrim($zakat->no_whatsapp, '0') : '6285230444463';
        $message = "*" . ucfirst($zakat->jenis_zakat) . " Baru*\n\n";
        $message .= "Nama Pembayar: {$zakat->nama_pembayar}\n";
        
        if ($zakat->jenis_zakat == 'fitrah') {
            $message .= "Jumlah Jiwa: {$zakat->jumlah_jiwa}\n\n";
            $message .= "*Daftar Muzakki:*\n";
            foreach ($zakat->muzakkis as $index => $muzakki) {
                $message .= ($index + 1) . ". {$muzakki->nama} ({$muzakki->hubungan_keluarga})\n";
            }
            $message .= "\nJenis Bayar: " . ucfirst($zakat->jenis_bayar) . "\n";
            if ($zakat->jenis_bayar == 'beras') {
                $message .= "Jumlah Beras: {$zakat->jumlah_beras} kg\n";
            } else {
                $message .= "Harga per Jiwa: Rp " . number_format($zakat->harga_per_jiwa, 0, ',', '.') . "\n";
                $message .= "Total Bayar: Rp " . number_format($zakat->total_bayar, 0, ',', '.') . "\n";
            }
        } else {
            $message .= "Total: Rp " . number_format($zakat->total_bayar, 0, ',', '.') . "\n";
            if ($zakat->keterangan) {
                $message .= "Keterangan: {$zakat->keterangan}\n";
            }
        }
        
        $message .= "Tanggal: " . date('d/m/Y H:i', strtotime($zakat->tanggal_bayar));
        
        $whatsappUrl = "https://wa.me/{$phoneNumber}?text=" . urlencode($message);

        return redirect()->route('zakat.index')
            ->with('success', 'Data zakat berhasil ditambahkan')
            ->with('whatsapp_url', $whatsappUrl);
    }

    public function edit(Zakat $zakat)
    {
        return view('zakat.edit', compact('zakat'));
    }

    public function update(Request $request, Zakat $zakat)
    {
        $rules = [
            'nama_pembayar' => 'required|string|max:255',
            'jenis_zakat' => 'required|in:fitrah,maal,shodaqoh',
            'no_whatsapp' => 'nullable|string|max:20',
            'tanggal_bayar' => 'required|date',
        ];

        if ($request->jenis_zakat == 'fitrah') {
            $rules['jumlah_jiwa'] = 'required|integer|min:1';
            $rules['jenis_bayar'] = 'required|in:tunai,beras';
            $rules['jumlah_beras'] = 'required_if:jenis_bayar,beras|nullable|numeric|min:0';
            $rules['harga_per_jiwa'] = 'required_if:jenis_bayar,tunai|nullable|numeric|min:0';
            $rules['muzakkis'] = 'required|array|min:1';
            $rules['muzakkis.*.nama'] = 'required|string|max:255';
            $rules['muzakkis.*.hubungan_keluarga'] = 'required|string|max:255';
        } else {
            $rules['jumlah_jiwa'] = 'nullable|integer|min:0';
            $rules['total_bayar'] = 'required|numeric|min:0';
            $rules['keterangan'] = 'nullable|string';
            $rules['muzakkis'] = 'required|array|min:1';
            $rules['muzakkis.*.nama'] = 'required|string|max:255';
            $rules['muzakkis.*.hubungan_keluarga'] = 'required|string|max:255';
        }

        $request->validate($rules);

        $data = $request->only(['nama_pembayar', 'jenis_zakat', 'no_whatsapp', 'keterangan', 'jumlah_jiwa']);
        $data['tanggal_bayar'] = $request->tanggal_bayar . ' ' . $zakat->tanggal_bayar->format('H:i:s');
        
        if ($request->jenis_zakat == 'fitrah') {
            $data['jumlah_jiwa'] = $request->jumlah_jiwa;
            $data['jenis_bayar'] = $request->jenis_bayar;
            $data['jumlah_beras'] = $request->jumlah_beras;
            $data['harga_per_jiwa'] = $request->harga_per_jiwa;
            
            if ($request->jenis_bayar == 'tunai') {
                $data['total_bayar'] = $request->jumlah_jiwa * $request->harga_per_jiwa;
            } else {
                $data['total_bayar'] = 0;
                $data['harga_per_jiwa'] = 0;
            }
        } else {
            $data['total_bayar'] = $request->total_bayar;
            $data['jumlah_jiwa'] = $request->jumlah_jiwa ?? 0;
            $data['jenis_bayar'] = 'tunai';
            $data['harga_per_jiwa'] = 0;
            $data['jumlah_beras'] = 0;
        }

        $zakat->update($data);

        $zakat->muzakkis()->delete();
        foreach ($request->muzakkis as $muzakki) {
            Muzakki::create([
                'zakat_id' => $zakat->id,
                'nama' => $muzakki['nama'],
                'hubungan_keluarga' => $muzakki['hubungan_keluarga']
            ]);
        }

        return redirect()->route('zakat.index')->with('success', 'Data zakat berhasil diperbarui');
    }

    public function destroy(Zakat $zakat)
    {
        $zakat->delete();
        return redirect()->route('zakat.index')->with('success', 'Data zakat berhasil dihapus');
    }
}