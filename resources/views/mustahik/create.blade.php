...
@section('title', 'Tambah Mustahik - Masjid Abal Qosim')
    ...
    <form action="{{ route('mustahik.store') }}" method="POST">
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div>
                ...
                <input type="text" class="w-full px-3 py-2 border border-gray-300 rounded focus:outline-none focus:border-masjid-green @error('nama') border-red-500 @enderror" id="nama" name="nama" value="{{ old('nama') }}" required>
                @error('nama')
            ...
            <div>
                ...
                <input type="text" class="w-full px-3 py-2 border border-gray-300 rounded focus:outline-none focus:border-masjid-green @error('no_hp') border-red-500 @enderror" id="no_hp" name="no_hp" value="{{ old('no_hp') }}">
                @error('no_hp')
                    <p class="text-red-500 text-xs mt-1">{{ $message }}</p>
                @enderror
            </div>
            <div class="md:col-span-2">
                ...
                <textarea class="w-full px-3 py-2 border border-gray-300 rounded focus:outline-none focus:border-masjid-green @error('alamat') border-red-500 @enderror" id="alamat" name="alamat" rows="3" required>{{ old('alamat') }}</textarea>
                @error('alamat')
                    <p class="text-red-500 text-xs mt-1">{{ $message }}</p>
                @enderror
            </div>
            <div>
                ...
                <select class="w-full px-3 py-2 border border-gray-300 rounded focus:outline-none focus:border-masjid-green @error('kategori') border-red-500 @enderror" id="kategori" name="kategori" required>
                    ...
                    <option value="gharim" {{ old('kategori') == 'gharim' ? 'selected' : '' }}>Gharim</option>
                    <option value="fisabilillah" {{ old('kategori') == 'fisabilillah' ? 'selected' : '' }}>Fisabilillah</option>
                    <option value="ibnu_sabil" {{ old('kategori') == 'ibnu_sabil' ? 'selected' : '' }}>Ibnu Sabil</option>
                </select>
                @error('kategori')
                    <p class="text-red-500 text-xs mt-1">{{ $message }}</p>
                @enderror
            ...
            <div>
                ...
                <textarea class="w-full px-3 py-2 border border-gray-300 rounded focus:outline-none focus:border-masjid-green @error('keterangan') border-red-500 @enderror" id="keterangan" name="keterangan" rows="3">{{ old('keterangan') }}</textarea>
                @error('keterangan')
                    <p class="text-red-500 text-xs mt-1">{{ $message }}</p>
                @enderror
            </div>
