...
@section('title', 'Tambah Zakat Maal - Masjid Abal Qosim')
@section('page-title', 'Tambah Zakat Maal')
        ...
        <div class="bg-white rounded-lg shadow">
            <div class="bg-gray-50 px-6 py-4 border-b-2 border-masjid-green rounded-t-lg">
                <h5 class="text-lg font-semibold text-gray-800 flex items-center">
                    ...
                    Tambah Data Zakat Maal
                </h5>
            </div>
            <div class="p-6">
                <form action="{{ route('zakat-maal.store') }}" method="POST">
                    @csrf
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                        <div class="mb-4">
                            ...
                            <input type="text" class="w-full px-3 py-2 border border-gray-300 rounded focus:outline-none focus:border-masjid-green @error('nama_pembayar') border-red-500 @enderror" 
                                   id="nama_pembayar" name="nama_pembayar" value="{{ old('nama_pembayar') }}" 
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                        <div class="mb-4">
                            ...
                            <input type="hidden" id="jumlah_harta" name="jumlah_harta" value="{{ old('jumlah_harta') }}" required>
                            @error('jumlah_harta')
                                <div class="text-red-500 text-sm mt-1">{{ $message }}</div>
                            @enderror
                        </div>
                        ...
                        <div class="mb-4">
                            ...
                            <input type="hidden" id="zakat_dibayar" name="zakat_dibayar" value="{{ old('zakat_dibayar') }}" required>
                            @error('zakat_dibayar')
                                <div class="text-red-500 text-sm mt-1">{{ $message }}</div>
                            @enderror
                        </div>
